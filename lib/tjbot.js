 ...
 */
TJBot.prototype._setupPreview = function() {
    winston.verbose("TJBot initializing Preview")
    var outputDir = this.configuration.see.camera.outputDir || '/tmp';
    var props = {
        height: this.configuration.see.camera.height,
        width: this.configuration.see.camera.width,
        mode: this.configuration.see.camera.mode,
        output: outputDir+'/preview.jpg',
        fullscreen: true,
        timeout: 0
    };

    if (this.configuration.see.camera.horizontalFlip) {
        props.hf = true;
    }
    if (this.configuration.see.camera.verticalFlip) {
        props.vf = true
    }

    this._preview = new RaspiCam(props);
}
...
TJBot.prototype._setupCamera = function() {
    ...
    this._camera = new Raspistill({
        ...
        noPreview: true,
        encoding: 'jpg',
        outputDir: this.configuration.see.camera.outputDir,
...
TJBot.prototype.sendAudio = function(stream, path) {
 ...
 */
TJBot.prototype.listen = function(listeners) {
    // make sure we can listen
    this._assertCapability('listen');
    if (typeof listeners !== 'object') {
        listeners = {
            onTranscript: listeners,
            onResult: undefined
        }
    }
    ...
    var SpeechToTextV1 = require('watson-developer-cloud/speech-to-text/v1');
    this._stt = new SpeechToTextV1({
        username: this.credentials.speech_to_text.username,
        password: this.credentials.speech_to_text.password,
        version: 'v1'
    });
    ...
    this._micTextStream.on('data', function(transcript) {
        ...
        if (listeners.onTranscript) {
            listeners.onTranscript(transcript);
        }
    });

    // Listen for events.
    // TODO contribute back a TJBot method that returns the micTextStream, so callers can listen
    if (listeners.onResult) {
        this._micTextStream.on('results', function(event) {
    ...
    return this._micRecognizeStream;
}
...
TJBot.prototype.stopListening = function() {
    ...
    // stop the mic
    this._mic.stop();
    require('child_process').spawn('/usr/bin/pkill', ['-9','arecord']);
 ...
 * return object.
 */
TJBot.prototype.see = function(opts) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        self.takePhoto().then(function(filePath) {
            resolve(self.recognizeObjectsInPhoto(filePath, opts))
        }).catch(function(err) {
            reject(err);
        });
    });
 ...
 */
TJBot.prototype.recognizeObjectsInPhoto = function(filePath, opts) {
    this._assertCapability('see');
    opts = opts || {};
    ...
    if (opts.classifier_ids) {
       params.classifier_ids = opts.classifier_ids
    } else {
...
TJBot.prototype.takePhoto = function(filePath) {
    ...
    if (filePath == null || filePath == "") {
        if (this.configuration.see.camera.outputDir) {
            filePath = photos.path({
                dir: this.configuration.see.camera.outputDir,
                prefix: 'img'
            });
        } else {
            filePath = temp.path({
    ...
    console.log("capturing image at path: " + path +"/"+name+".jpg");
 ...
 */
TJBot.prototype.startPreview = function() {
    this._assertCapability('see');

    // capture 'this' context
    var self = this;
    return new Promise(function(resolve, reject) {
        winston.verbose("Starting Preview");
        resolve(self._preview.start());
    });
}

TJBot.prototype.stopPreview = function() {
    this._assertCapability('see');

    // capture 'this' context
    var self = this;
    return new Promise(function(resolve, reject) {
        winston.verbose("Exiting Preview");
        self._preview.stop();
        self._preview.on('exit', function(){
            resolve();
        });
        require('child_process').spawn('/usr/bin/pkill', ['-9','raspistill']);
    });
}
 ...
 */
TJBot.prototype.speak = function(message, opts) {
    ...
    if (opts.voice !== undefined) {
        voice = opts.voice;
    } else if (this.configuration.speak.voice != undefined) {
        ...
        temp.open('tjbot', function(err, info) {
            ...
            self._tts.synthesize(utterance)
                ...
                .on('close', function() {
                    ...
                    var playOpts = opts.shouldPauseResume === undefined ? undefined : opts;
                    resolve(self.play(info.path, playOpts));
                });
        });
    });
}
 ...
 */
TJBot.prototype.play = function(soundFile, opts) {
    opts = opts || { shouldPauseResume: true };
    // capture 'this' context
    var self = this;
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (self.configuration.speak.player) {
            speakerOptions.player = self.configuration.speak.player;
        }

        if (self.configuration.speak.speakerDeviceId) {
            speakerOptions.device = self.configuration.speak.speakerDeviceId;
        }
